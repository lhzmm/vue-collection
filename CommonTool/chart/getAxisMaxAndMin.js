 ...
 */
export default function getAxisMaxAndMin(option = {}) {
    var _a, _b, _c, _d;
    // 参数检查
    ...
    if (min === max) {
        const val = min;
        if (((_a = String(val).split('.')[1]) === null || _a === void 0 ? void 0 : _a.length) > fixedLength) {
    ...
    if (((_b = String(axisMin).split('.')[1]) === null || _b === void 0 ? void 0 : _b.length) > fixedLength) {
        let offset = 0;
        if (axisMin < 0) {
    ...
    if (((_c = diffStr.split('.')[1]) === null || _c === void 0 ? void 0 : _c.length) > fixedLength) {
        const matched = diffStr.match(new RegExp(`\\d+\\.\\d{${fixedLength}}`));
        axisMax = add(axisMin, multiply(splitNumber, add(matched[0], minInterval)));
    }
    let interval = String(getInterval({ max: axisMax, min: axisMin, splitNumber }));
    if (((_d = interval.split('.')[1]) === null || _d === void 0 ? void 0 : _d.length) > fixedLength) {
